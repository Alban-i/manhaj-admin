...
export interface ThemeEventNested extends ThemeEvent {
  children: ThemeEvent[];
}

// Helper function to build nested structure from flat list
export function nestEvents(events: ThemeEvent[]): ThemeEventNested[] {
  // Create a map for quick lookup
  const eventMap = new Map<string, ThemeEventNested>();
  ...
  const rootEvents: ThemeEventNested[] = [];
